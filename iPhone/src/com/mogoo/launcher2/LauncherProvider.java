...
import com.mogoo.launcher2.utils.MogooFavoritesUtils;
...
public class LauncherProvider extends ContentProvider {
    ...
    static final String AUTHORITY = "com.mogoo.launcher2.settings";
    
    public static final String TABLE_FAVORITES = "favorites";
    static final String TABLE_ACTIVE = "APP_ACTIVE";
    ...
    public static class DatabaseHelper extends SQLiteOpenHelper {
        ...
        public void onCreate(SQLiteDatabase db) {
            ...
            db.execSQL("CREATE TABLE APP_ACTIVE (" +
                    "_id INTEGER PRIMARY KEY," +
                    "package TEXT," +
                    "activeDate INTEGER," +
                    "updateDate INTEGER," +
        ...
        private int copyFromCursor(SQLiteDatabase db, Cursor c) {
            ...
            final int titleResourceIndex = c.getColumnIndexOrThrow(LauncherSettings.Favorites.TITLE_RESOURCE);
            //end
            ...
            while (c.moveToNext()) {
                ...
                //add byeben 2012-3-30
                values.put(LauncherSettings.Favorites.TITLE_RESOURCE, c.getString(titleResourceIndex));
                //end
...
//            List<ComponentName> loadsItems = new ArrayList<ComponentName>() ;
...
//                            LauncherSettings.Favorites.CONTAINER_DESKTOP);
                    ...
                    try{
                        container = Integer.parseInt(a.getString(R.styleable.Favorite_container));
                    }catch(Exception e){
                    ...
                    //********end add by 张永辉 2010-12-9
                    
                    //如下有此AIP为2.2版本才支持
                    if(Mogoo_GlobalConfig.OS_VERSION_2_2.equals(Mogoo_GlobalConfig.getOsVersion())){
                        ...
                        //add by huangyue 2012-6-12
                        else if (TAG_FOLDER.equals(name)) {
                        		i += MogooFavoritesUtils.addFolder(parser, db, values, mContext, this, a);
                           }
                        //end
                    }
...
//                    if(added&&TAG_FAVORITE.equals(name)){
//                        ActivityInfo info;
...
//                    }
               ...
               //add by 张永辉 2010-12-28 确认是否抽屉里的软件图标全加载到桌面上去了
               //查询出己加载到桌面和工具栏中的快捷方式
//                SQLiteDatabase db2 = this.getReadableDatabase() ;
//               List<ComponentName> loadsItems = new ArrayList<ComponentName>() ;
//               Cursor c = db2.query(TABLE_FAVORITES, null, null, null, null, null, null) ;
//               final int intentIndex = c.getColumnIndexOrThrow(LauncherSettings.Favorites.INTENT);
//               Log.d(TAG, c.getCount()+","+intentIndex);
//               while (c.moveToNext()) {
...
//                           if(intent!=null ){
//                               loadsItems.add(intent.getComponent());
//                           }
//                       } catch (URISyntaxException e) {
//                           continue;
//                       }
...
//                       appInfos.add(new ApplicationInfo(ri, ((LauncherApplication)(mContext.getApplicationContext())).mIconCache));
//                   }
//                   List<ApplicationInfo> hasNotAddToDeskList = new ArrayList<ApplicationInfo>() ;
//                   for(ApplicationInfo ai:appInfos){
//                       if(loadsItems!=null){
//                           if(ai.intent!=null&&!loadsItems.contains(ai.intent.getComponent())){
//                               hasNotAddToDeskList.add(ai);
//                           }
//                       }
//                   }
//                   
//                   //把没有加入到桌面中的应用加到桌面上去
//                   for(ApplicationInfo ai:hasNotAddToDeskList){
//                       addToDesk(ai,mContext) ;
//                   }
//                   
//               }
               //end 
        ...
        public boolean addAppShortcut(SQLiteDatabase db, ContentValues values, TypedArray a,
                ...
                if(titleResId>0)
                {
                   //test
             	   ...
             	   if(title!=null && !title.equals(""))
                    {
                 	   ...
                 	   //Log.d("LauncherProvider", "Favorites.TITLE =" + title);
                    }else
                    {
                 	   values.put(Favorites.TITLE, info.loadLabel(packageManager).toString());
                 	   //Log.d("LauncherProvider", "Favorites.TITLE =" + title);
             	   ...
             	   //add by yeben 2012-3-30
             	   values.put(Favorites.TITLE_RESOURCE, r.getResourceName(titleResId));
             	   //end
                }else
                {
             	   values.put(Favorites.TITLE, info.loadLabel(packageManager).toString());
...
//                    values.put(Favorites.ICON_PACKAGE, mContext.getPackageName());
            	...
            	//--------------end-------------------------------------------------------- 
                //add by yeben 2012-4-1
                if(titleResId>0 || iconResId>0){
                	values.put(Favorites.ICON_TYPE, Favorites.ICON_TYPE_RESOURCE);
                	values.put(Favorites.ICON_PACKAGE, mContext.getPackageName());
                }
                //end
                ...
                //***********add by 张永辉 2010-12-9
                //设置默认值
                if(values.get(Favorites.IS_SYSTEM)==null){
                    values.put(Favorites.IS_SYSTEM, Favorites.NOT_SYSTEM_APP);
                }
                if(values.get(Favorites.APP_TYPE)==null){
                    values.put(Favorites.APP_TYPE, Favorites.APP_TYPE_OTHER);
                }
                //*******end add by 张永辉 2010-12-9
