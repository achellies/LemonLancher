...
public class Hanzi2Pinyin1 {
	...
	private static Hanzi2Pinyin1 sInstance;
	...
	protected Hanzi2Pinyin1(boolean hasChinaCollator) {
		mHasChinaCollator = hasChinaCollator;
	}

	public static Hanzi2Pinyin1 getInstance() {
		synchronized(Hanzi2Pinyin1.class) {
			if (sInstance != null) {
				return sInstance;
			}
			...
			for (int i = 0; i < locale.length; i++) {
				if (locale[i].equals(Locale.CHINA)) {
					sInstance = new Hanzi2Pinyin1(true);
					return sInstance;
				}
			}
			Log.w(TAG, "There is no Chinese collator, HanziToPinyin is disabled");
			sInstance = new Hanzi2Pinyin1(false);
			return sInstance;
		}
	}
